...
final class WalletViewModel: ObservableObject {
    ...
    init() {
        ...
        #if DEBUG
        // Test/demo-only: seeds a plain fixture credential set and jumps
        // straight past login to the credentials tab, bypassing the wallet
        // session entirely - the credential card stack (`CredentialStack`,
        // `CredentialsView`) is a pure view over `credentials`/`walletState`,
        // so exercising its tap/long-press/drag interactions (manually via
        ...
        // directly over a fixture `CredentialWithInstances` list with no
        // wallet session either.
        if ProcessInfo.processInfo.environment["SIROS_SAMPLE_APP_FIXTURE_CREDENTIALS"] != nil {
            walletState = .ready
    ...
    /// `SIROS_SAMPLE_APP_FIXTURE_CREDENTIALS` above.
